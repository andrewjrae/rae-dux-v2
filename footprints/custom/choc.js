...
module.exports = {
    ...
    body: p => {
      ...
      const make_trace = (x1, y1, x2, y2, side) => {
        return `
          (segment (start ${ adjust_point(x1, y1) }) (end ${ adjust_point(x2, y2) }) (width 0.25) (layer ${ side }.Cu) (net 1))
        `
      }
      ...
      let traces = '';
      if(p.traces && p.hotswap && p.hotswap_tht && p.reverse) {
        frompad_x = 3.275 - 0.2;
        frompad_y = -5.95;
        fromhole_x = 5;
        fromhole_y = 3.8;

        frommid_y = (frompad_y + fromhole_y * 3) / 4;

        traces += make_trace(frompad_x, frompad_y, frompad_x, frommid_y, 'F');
        traces += make_trace(frompad_x, frommid_y, fromhole_x, fromhole_y, 'F');
        traces += make_trace(-frompad_x, frompad_y, -frompad_x, frommid_y, 'B');
        traces += make_trace(-frompad_x, frommid_y, -fromhole_x, fromhole_y, 'B');

        traces += make_trace(fromhole_x, fromhole_y, -fromhole_x, fromhole_y, 'F');
        ...
        tomid_x = (topad_x * 3 + tohole_x) / 4;
        tomid_y = (topad_y + tohole_y * 3) / 4;

        traces += make_trace(topad_x, topad_y, topad_x, tomid_y, 'F');
        traces += make_trace(topad_x, tomid_y, tomid_x, tohole_y, 'F');
        traces += make_trace(tomid_x, tohole_y, tohole_x, tohole_y, 'F');
        if(!p.assume_ground_fill) {
          traces += make_trace(-topad_x, topad_y, -topad_x, tomid_y, 'B');
          traces += make_trace(-topad_x, tomid_y, -tomid_x, tohole_y, 'B');
          traces += make_trace(-tomid_x, tohole_y, -tohole_x, tohole_y, 'B');
        }
      }
